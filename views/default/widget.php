<?= $args['before_widget']; ?>
<?= $args['before_title']; ?>
<?= (!empty($args['titlelink']) && $args['titlelink'] == "Y") ? '<a href="' . $args['titlelinkurl'] . '" title="' . $args['title'] . '">' : ''; ?>
<?= $args['title']; ?>
<?= (!empty($args['titlelink']) && $args['titlelink'] == "Y") ? '</a>' : ''; ?>
<?= $args['after_title']; ?>
<?php if (!empty($args['parent'])) : ?>

<ul>
  <li> <a href="<?= $args['parent']['href']; ?>" title="<?= $args['parent']['title']; ?>">
    <?= $args['parent']['title']; ?>
    </a>
    <?php endif; ?>
    <ul>
      ...
      <li>
        ...
        <a class="rsswidget" href="<?= get_option('home'); ?>/?feed=rss2&amp;cat=<?= $item['cat_ID']; ?>" title="<?php _e('RSS Feed', $this -> plugin_name); ?>"> <img style="border:none !important;" src="<?= get_option('siteurl'); ?>/wp-includes/images/rss.png" alt="<?php _e('rss', $this -> plugin_name); ?>" />
        <?php }?>
        </a>
        <?php endif; ?>
        <a href="<?= $item['href']; ?>" title="<?= $item['title']; ?>">
        <?= (!empty($args['max_length']) && strlen($item['title']) > $args['max_length']) ? substr($item['title'], 0, $args['max_length']) . '...' : $item['title']; ?>
        </a>
